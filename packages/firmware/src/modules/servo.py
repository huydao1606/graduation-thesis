...
class Servo:
    ...
    async def drop(self, slot: str, quantity: int, timeout_per_pill: int = 8) -> bool:
        ...
        for i in range(quantity):
            # LƯU LẠI GIÁ TRỊ ĐẾM BAN ĐẦU (Không reset về 0 nữa)
            initial_drop_count = self._states.drop_count
            ...
            while time.ticks_diff(time.ticks_ms(), start_time) < timeout_ms:
                # NẾU BIẾN ĐẾM TĂNG LÊN -> CHẮC CHẮN CÓ THUỐC RỚT
                if self._states.drop_count > initial_drop_count:
                    pill_dropped = True
                    break
            ...
            if not pill_dropped:
                print(f"[ERROR] Slot {slot} timeout! Initial: {initial_drop_count}, Current: {self._states.drop_count}")
                return False
